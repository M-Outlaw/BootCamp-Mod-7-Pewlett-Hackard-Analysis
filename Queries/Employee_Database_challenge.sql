...
-- Determine the percent per title for the retirement list
SELECT count(title),
title,
ROUND(count(title) * 100.0 / (SELECT count(title) FROM unique_titles),0) as Percentage
FROM unique_titles
GROUP BY title
ORDER BY count(title) DESC;

-- Count the number per department for the retirement list
...
on d p.dept_no = de.dept_no
GROUP BY dp.dept_name
ORDER BY count(de.emp_no) DESC;

-- Count the number of mentors per title
SELECT count(title),
title
FROM mentor_list
GROUP BY title
...
-- Determine the percent per title for the mentor list
SELECT count(title),
title,
ROUND(count(title) * 100.0 / (SELECT count(title) FROM mentor_list),0) as Percentage
FROM mentor_list
GROUP BY title
ORDER BY count(title) DESC;

-- Count the number per department for the mentor list
SELECT count(de.emp_no), 
dp.dept_name
FROM dept_emp as de
INNER JOIN mentor_list as ml
on de.emp_no = ml.emp_no
INNER JOIN departments as dp
on d p.dept_no = de.dept_no
GROUP BY dp.dept_name
ORDER BY count(de.emp_no) DESC;